...
import 'package:giff_dialog/src/base_dialog.dart';

/// Widget that displays Giff Dialog with local asset.
///
/// You need local asset for this type of Giff Dialog.
...
/// ```
/// Pass the asset to [AssetGiffDialog] via [image] prop using Flutter's `Image.asset(name)` widget.
/// ```
/// AssetGiffDialog(
///   title: Text('Example'),
...
///   ...,
/// );
class AssetGiffDialog extends StatelessWidget {
  const AssetGiffDialog({
    Key? key,
    required this.image,
    required this.title,
  ...
  /// Preferably with `fit: BoxFit.cover` property to cover entire top part of Giff Dialog.
  /// All properties of Image widget are directly passed to Giff Dialog so feel free to experiment.
  final Image image;
